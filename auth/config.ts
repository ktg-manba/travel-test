...
if (
  ...
  console.log("Google Auth is disabled or missing configuration");
}
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          return {
            id: authData.user.id,
            email: authData.user.email || credentials.email as string,
            name: authData.user.user_metadata?.name || authData.user.email?.split("@")[0] || "",
            image: authData.user.user_metadata?.avatar_url || "",
          };
        } catch (e) {
          console.log("Supabase authorize error:", e);
          return null;
        }
      },
    })
...
export const authOptions: NextAuthConfig = {
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      console.log("Session Callback - Token:", token?.user ? "User exists" : "No User");
      if (token && token.user) {
    ...
    async jwt({ token, user, account }) {
      ...
      if (account) {
        console.log("JWT Callback - Initial Signin:", account.provider);
      }
      ...
      try {
        if (user && user.email) {
          const dbUser: User = {
            ...
            signin_provider: account?.provider || "supabase-email",
          ...
          try {
            ...
            console.log("JWT Callback - User saved:", savedUser.uuid);
